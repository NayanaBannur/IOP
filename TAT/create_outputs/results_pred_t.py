...
from TAT.train_pred_t import acc_metric, compute_metric, bleu_metric, meteor_metric, kendall_tau_metric, spearman_metric
...
all_results = []
folders = ['SMS-A-3-t', 'CollegeMsg-4-t']  # 'CollegeMsg-3-t',
n = [3, 4]
...
for i, folder in enumerate(folders):
    ...
    for epoch in range(50):
        ...
        for split in ['val', 'test']:
            ...
            results = compute_metric(predictions, labels, metrics, n=n[i])
    ...
    all_results.append({'Name': folder,
                        'Accuracy': np.round(np.mean(test_acc), 3),
