...
from TAT.train_pred_t import acc_metric, compute_metric, bleu_metric, kendall_tau_metric, spearman_metric, meteor_metric
...
for epoch in range(50):
    ...
    for split in ['val', 'test']:
        ...
        for t in range(6):
            ...
            predictions.append(torch.load(path + f'{split}_predictions_{epoch}.pt'))
            labels.append(torch.load(path + f'{split}_labels_{epoch}.pt'))
...
all_results = []
all_results.append({'Accuracy': np.round(np.mean(test_acc), 3),
