...
from .utils import lookup_table_n, inverse_lookup
...
def f1_score_metric(predictions, labels, num_nodes=3):
    # Only for single timestamp
    new_predictions = []
    for i in range(0, predictions.shape[1], num_nodes):
        _, idx = predictions[:, i:i + num_nodes].topk(2, dim=1)
        new_predictions.append(torch.nn.functional.one_hot(idx, num_nodes).sum(dim=1))
    ...
    if isinstance(labels, Tensor):
        labels = labels.cpu().numpy()
    predictions_list = [inverse_lookup(predictions[i].tolist(), num_nodes) for i in range(len(predictions))]
    labels_list = [inverse_lookup(labels[i].tolist(), num_nodes) for i in range(len(labels))]
    macro_f1_score = f1_score(predictions_list, labels_list, average='macro')
...
def kendall_tau_metric(predictions, labels, num_nodes=3):
    new_predictions = []
    for i in range(0, predictions.shape[1], num_nodes):
        _, idx = predictions[:, i:i + num_nodes].topk(2, dim=1)
        new_predictions.append(torch.nn.functional.one_hot(idx, num_nodes).sum(dim=1))
    ...
    for i in range(0, predictions.shape[1], num_nodes):
        ...
        pred = predictions[:, i:i + num_nodes]
        for j in range(len(predictions)):
            new_predictions[i // num_nodes].append(inverse_lookup(pred[j].tolist(), num_nodes))
    for i in range(0, labels.shape[1], num_nodes):
        new_labels.append([])
        lab = labels[:, i:i + num_nodes]
        for j in range(len(labels)):
            new_labels[i // num_nodes].append(inverse_lookup(lab[j].tolist(), num_nodes))
    predictions_list = np.array(new_predictions).T
    labels_list = np.array(new_labels).T
    tau = 0.0
    pval = 0.0
    for i in range(predictions.shape[0]):
        kendall_tau = scipy.stats.kendalltau(predictions_list[i], labels_list[i])
...
def spearman_metric(predictions, labels, num_nodes=3):
    new_predictions = []
    for i in range(0, predictions.shape[1], num_nodes):
        _, idx = predictions[:, i:i + num_nodes].topk(2, dim=1)
        new_predictions.append(torch.nn.functional.one_hot(idx, num_nodes).sum(dim=1))
    ...
    with warnings.catch_warnings():
        ...
        for i in range(predictions.shape[0]):
            metric_val = scipy.stats.spearmanr(predictions_list[i], labels_list[i])
...
def bleu_metric(predictions, labels, num_nodes=3, ngram=3):
    if ngram == 3:
    ...
    for i in range(0, labels.shape[1], num_nodes):
        ...
        for j in range(len(labels)):
            new_labels[i // num_nodes].append(inverse_lookup(lab[j].tolist(), num_nodes))
    predictions_list = np.array(new_predictions).T.tolist()
    labels_list = np.array(new_labels).T.tolist()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        bleu = 0
        for i in range(len(labels)):
            bleu += sentence_bleu([labels_list[i]],
                                  predictions_list[i],
                                  weights=weights)
        bleu /= len(labels)
        return bleu
...
def meteor_metric(predictions, labels, num_nodes=3):
    new_predictions = []
    for i in range(0, predictions.shape[1], num_nodes):
        _, idx = predictions[:, i:i + num_nodes].topk(2, dim=1)
        new_predictions.append(torch.nn.functional.one_hot(idx, num_nodes).sum(dim=1))
    ...
    with warnings.catch_warnings():
        ...
        meteor = 0
        for i in range(len(labels)):
            meteor += meteor_score([[str(s) for s in labels_list[i]]],
                                   [str(s) for s in predictions_list[i]])
        meteor /= len(labels)
        return meteor
...
def compute_metric(predictions, labels, metrics, n=3):
    ...
    for key, f in metrics.items():
        if key in ['acc', 'macro_f1', 'bleu', 'BLEU-2', 'BLEU-3', 'meteor', 'kendall_tau', 'spearman']:
