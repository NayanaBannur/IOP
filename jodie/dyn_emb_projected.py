...
# avg_train_ts = 0
# test_increments = np.full(num_users, avg_train_ts)
# test_increments = np.insert(np.cumsum(test_increments), 0, 0)[:-1]
# test_increments = scale(np.array(test_increments) + 1)
test_increments = np.random.normal(0, 1, num_users)
test_increments.sort()

test_increments = torch.tensor(test_increments, dtype=torch.float32).reshape(-1, 1)
...
with torch.no_grad():
    user_embeddings = temp_model.forward(user_embeddings, None, timediffs=test_increments,
                                         features=None, select='project')
...
test_dataset = JodieDataset(set_indice_length, test_set_indices[:n_test], test_labels[:n_test], user_embeddings)
...
for run in range(10):
    ...
    for epoch in tqdm(range(50)):
        ...
        with torch.no_grad():
            ...
            test_metrics = metrics(preds, labels)
            test_metrics = {'test' + '_' + k: v for k, v in test_metrics.items()}
    ...
    for key in ['acc', 'bleu', 'meteor', 'f1']:
        ...
        best_metrics[key] = all_metrics[f"test_{key}"][idx]
    for key in ['kt', 'sp']:
        idx = np.argpartition(-all_metrics[f"val_{key}_corr"], 1)[0]
        best_metrics[f'{key}_corr'] = all_metrics[f"test_{key}_corr"][idx]
        best_metrics[f'{key}_pval'] = all_metrics[f"test_{key}_pval"][idx]
    best_metrics['run'] = run
    print(best_metrics)
    all_results.append(best_metrics)

all_results = pd.DataFrame(all_results)
all_results.to_csv('./outputs/results_dyn_emb_projected.csv', index=False)
